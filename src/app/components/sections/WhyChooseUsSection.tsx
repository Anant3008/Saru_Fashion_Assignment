        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
          {WHY_US.map((item, i) => (
            <div key={i} className="flex gap-5 items-start group">
              <div className="w-14 h-14 rounded-2xl bg-[#F0F8F8] flex items-center justify-center text-2xl flex-shrink-0 group-hover:bg-primary transition-all duration-300 shadow-sm">
                <item.icon className="h-6 w-6 text-primary transition-colors duration-300 group-hover:text-white" strokeWidth={1.75} />
              </div>
              <div>
                <h3 className="font-semibold text-foreground text-[15px] mb-1.5 group-hover:text-primary transition-colors">
                  {item.title}
                </h3>
                <p className="text-foreground/58 text-[13px] leading-relaxed">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
