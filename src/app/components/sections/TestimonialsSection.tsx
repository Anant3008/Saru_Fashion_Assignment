        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {TESTIMONIALS.map((t, i) => (
            <div key={i} className="bg-white/[0.09] backdrop-blur-sm border border-white/15 rounded-3xl p-7 hover:bg-white/[0.14] transition-all duration-200">
              <div className="playfair text-5xl text-secondary leading-none mb-1">"</div>
              <p className="text-white/82 text-[13px] leading-[1.8] mt-3 mb-5">{t.text}</p>
              <Stars n={t.rating} />
              <div className="flex items-center gap-3 mt-4 pt-4 border-t border-white/10">
                <div className="w-10 h-10 rounded-full taupe-gradient flex items-center justify-center text-foreground text-xs font-bold flex-shrink-0">
                  {t.initials}
                </div>
                ...
                <span className="bg-secondary/20 text-secondary text-[9px] font-bold tracking-widest uppercase px-2.5 py-1 rounded-full flex-shrink-0">
                  {t.tag}
                </span>
              </div>
            </div>
